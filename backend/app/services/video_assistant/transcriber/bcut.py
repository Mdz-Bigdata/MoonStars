...
class BcutTranscriber(Transcriber):
    ...
    def _upload(self, file_path: str) -> None:
        ...
        resp = resp.json()
        if resp.get("code") != 0:
            raise Exception(f"申请上传失败: {resp.get('message')}")
    ...
    def __commit_upload(self) -> None:
        """提交上传数据"""
        # 注意: model_id 为 "8" 是 B 站最新的 ASR 引擎 ID
        data = json.dumps({
        ...
        resp.raise_for_status()
        resp = resp.json()
        logger.info(f"Bcut 提交返回: {resp}")
        
        if resp.get("code") != 0:
            error_msg = f"上传提交失败: {resp.get('code')} - {resp.get('message', '未知错误')}"
            logger.error(error_msg)
            # 如果是 139201，可能是 B 站内部服务波动，但在代码层面我们确保参数齐全
