...
from app.services.video_assistant.downloaders.base import Downloader
...
from app.services.video_assistant.db.video_task_dao import delete_task_by_video, insert_video_task
from app.services.video_assistant.enums.exception import NoteErrorEnum, ProviderErrorEnum
from app.services.video_assistant.enums.task_status_enums import TaskStatus
from app.services.video_assistant.enums.note_enums import DownloadQuality
...
from app.services.video_assistant.models.audio_model import AudioDownloadResult, NoteResult
from app.services.video_assistant.models.transcriber_model import TranscriptResult, TranscriptSegment
from app.services.video_assistant.constant import SUPPORT_PLATFORM_MAP
...
from app.services.video_assistant.transcriber.transcriber_provider import get_transcriber, _transcribers
from app.services.video_assistant.utils.note_helper import replace_content_markers
from app.services.video_assistant.utils.status_code import StatusCode
from app.services.video_assistant.utils.video_helper import generate_screenshot
from app.services.video_assistant.utils.video_reader import VideoReader
...
class NoteGenerator:
    ...
    def generate(
        ...
        try:
            ...
            self._update_status(task_id, TaskStatus.PARSING, video_url=str(video_url))
    ...
    def _get_gpt(self, model_name: Optional[str], provider_id: Optional[str]) -> GPT:
        ...
        # ⛳ 增加环境变量兜底，解决数据库中 API Key 为空导致的 401 错误
        api_key = provider.get("api_key")
        if not api_key:
            env_key_name = f"{provider['id'].upper()}_API_KEY"
            api_key = os.getenv(env_key_name)
            
            # 针对特定平台的兜底逻辑
            if not api_key:
                provider_id_lower = provider['id'].lower()
                if provider_id_lower == 'deepseek':
                    api_key = os.getenv("DEEPSEEK_API_KEY")
                elif provider_id_lower == 'openai':
                    api_key = os.getenv("GPT_API_KEY") # 很多用户习惯命名为 GPT_API_KEY
            
            if api_key:
                logger.info(f"从环境变量中获取到 {provider['id']} 的 API Key")
            else:
                error_msg = f"供应商 {provider['id']} 缺失 API Key，且未在数据库或环境变量({env_key_name})中设置"
                logger.error(error_msg)
                raise ProviderError(code=ProviderErrorEnum.INVALID_KEY, message=error_msg)
    ...
    def _update_status(self, task_id: Optional[str], status: Union[str, TaskStatus], message: Optional[str] = None, video_url: Optional[str] = None):
        """
    ...
    def _transcribe_audio(
        ...
        try:
            ...
            if transcript is None:
                raise Exception("音频转写返回结果为空，请检查转写器配置或日志")
